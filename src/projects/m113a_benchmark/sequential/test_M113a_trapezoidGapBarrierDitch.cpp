...
ChVector3d initLoc(-0.1, 0.0, 0.75);
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 2)
        angle = std::atof(argv[2]) * CH_PI / 180.0;
    if (argc > 3)
        ...
        if (step_number % render_steps == 0) {
            ...
            if (state_output) {
                ...
                ChVector3d vel_driver_abs =
                    m113.GetChassisBody()->GetFrameRefToAbs().PointSpeedLocalToParent(driver_pos);
    ...
    if (state_output) {
        ...
        sprintf(filename, "%s/output_%d_%d_%d.dat", out_dir.c_str(), int(std::round(width / 0.0254)),
                int(std::round(angle * 180 / CH_PI)), int(std::round(height / 0.0254)));
