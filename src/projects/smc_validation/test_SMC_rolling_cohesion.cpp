...
void WriteData(std::ofstream& dat, ChSystemMulticoreSMC* msystem, const std::string& str) {
    for (int i = 0; i < msystem->GetBodies().size(); ++i) {
        ...
        dat << str.c_str() << "," << body->GetIdentifier() << "," << msystem->GetChTime() << "," << radius << ","
            ...
            << body->GetPosDt2().y() << "," << body->GetPosDt2().z() << "," << body->GetAngAccParent().x() << ","
            << body->GetAngAccParent().y() << "," << body->GetAngAccParent().z() << "," << msystem->GetBodyContactForce(body).x
...
int main(int argc, char* argv[]) {
    ...
    for (int f = 0; f < fmodels.size(); ++f) {
        ...
        while (time < time_sim) {
            ...
            if (KEthresh) {
                std::cout << "[settling " << fmodels[f].c_str() << "] KE exceeds threshold at t = " << time << "\n";
                break;
            }
        }
        ...
        ChVector3d init_w = ChVector3d(0, 0, 1);
        ...
        double wvel_diff = abs(body2->GetAngVelParent().z() + body1->GetAngVelParent().z());
    }
